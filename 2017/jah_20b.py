...
def main(s):
  ...
  s = s.strip().split('\n')
  particles = {i:parse(line) for (i,line) in enumerate(s)}

  steps = 0
  while not steady(particles):
    step(particles)
...
def step(particles):
  ...
  for p in particles.values():
    p['v'] = map(sum,zip(p['v'],p['a']))
    p['p'] = map(sum,zip(p['p'],p['v']))

def collisions(particles):

  ps = sorted([(i,tuple(p['p'])) for (i,p) in particles.iteritems()])
  last = (-1,None)
  for p in ps:
    if p[1]==last[1]:
      del particles[p[0]]
      try:
        del particles[last[0]]
      except KeyError:
        pass
    else:
      last = p

def steady(particles):

  for i in range(0,3):
    ps = sorted(particles.values(),key=lambda x:x['p'][i])
    last = None
    for p in ps:
      (v,a) = (p['v'][i],p['a'][i])
      if last is None or (last[0]<=v and last[1]<=a):
        last = (v,a)
      else:
        return False
  return True
