...
def main(s):
  ...
  nodes = map(parse,s.strip().split('\n'))
  tree = Tree()
  last = len(nodes)+1
  while len(nodes)<last:
    last = len(nodes)
    i = 0
    while i<len(nodes):
      if tree.add(*nodes[i]):
  ...
  try:
    ...
    s = str(e.node.parent)+'\n'
    s += '-'*20+'\n'
    for (c,w) in zip(e.node.parent.children,e.child_weights):
      s += '%s | %s\n' % (w,c)
    s += '-'*20+'\n'
    s += '%s should be %s' % (e.node.name,e.node.weight+e.correction)
    return s
...
def parse(s):
  ...
  name = s[0]
  weight = int(s[1][1:-1])
  children = [] if len(s)<3 else s[3:]
  return (name,weight,children)

class UnbalancedError(Exception):

  def __init__(self,node,child_weights,correction):

    super(UnbalancedError,self).__init__()
    self.node = node
...
class Tree(object):
  ...
  def add(self,name,weight,children=None):

    for child in children:
      if child not in self.nodes:
        return False

    children = map(self.nodes.get,children)
    level = 1 if not children else max(children,key=lambda c:c.level).level+1
    node = Node(name,weight,children,level)
  ...
  def validate(self,node=None):
    ...
    for child in node.children:
      child_weights.append(self.validate(child))
    if len(set(child_weights))>1:
      for (i,w) in enumerate(child_weights):
        if child_weights.count(w)==1:
          break
      correct = child_weights[(i+1)%len(child_weights)]
      correction = correct-child_weights[i]
      raise UnbalancedError(node.children[i],child_weights,correction)

    return node.weight+sum(child_weights)

  def set_levels(self,node=None,current=None):

    node = node or self.root
    current = current or self.height

    node.level = current
    for child in node.children:
      self.set_levels(child,current-1)

class Node(object):

  def __init__(self,name,weight,children=None,level=0):
  ...
  def __str__(self):
    ...
    return '%s @%s w%s c%s' % (self.name,self.level,self.weight,len(self.children))
